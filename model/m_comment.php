...
class m_Comment extends DBconnect
{
	...
	{
		$sql = "SELECT * FROM tblcomment where post_id = ?";
		$this->setQuery($sql);
	...
	{
		$sql = "SELECT * FROM tblcomment where user_id = ? ORDER BY created_at DESC LIMIT 5";
		$this->setQuery($sql);
	...
	public function getUserRecentCommentFromClass($id, $in_class)
	{
		$sql = "SELECT c.post_id as post_id, 
				...
				where c.user_id = ? AND p.in_class = ?
	...
	public function EditComment($comment_id, $post_id, $content, $user_id)
	{
		...
		$sql = "UPDATE tblcomment SET post_id = ?,content = ? ,user_id = ? where comment_id= ?;";
		$this->setQuery($sql);

		$result = $this->execute(array($post_id, $content, $user_id, $comment_id));
		if ($result) {
			return $this->getLastInserted();
		} else
			return false;
	...
	public function DeleteComment($comment_id)
	{
		...
		$sql = "DELETE FROM tblcomment where comment_id= ?;";
		$this->setQuery($sql);
	...
	{
		$sql = "SELECT  post_id,content,user_id,created_at FROM tblcomment WHERE comment_id= ?;";
		$this->setQuery($sql);
